...
from sb3_contrib.common.recurrent.buffers import RecurrentDictRolloutBuffer, RecurrentRolloutBuffer, pad ,pad_and_flatten, create_sequencers 

from sb3_contrib.common.recurrent.policies import RecurrentActorCriticPolicy
#from sb3_contrib.common.recurrent.type_aliases import RNNStates
from sb3_contrib.ppo_recurrent.policies import CnnLstmPolicy, MlpLstmPolicy, MultiInputLstmPolicy
from myGym.stable_baselines_mygym.policies import RNNStatesCOM
...
class RecurrentRolloutBufferSamples2(NamedTuple):
    ...
    coms: th.Tensor
    coms_truth: th.Tensor
    mask: th.Tensor


class ExtendedRecurrentRolloutBuffer(RecurrentRolloutBuffer):
    def __init__(self, buffer_size, observation_space, action_space, hidden_state_dim,   device, gae_lambda=1, gamma=0.99,n_envs=1):
        super().__init__(buffer_size, observation_space, action_space, hidden_state_dim,device,  gae_lambda, gamma, n_envs)
        # Initialize the coms buffer with correct dimensions
        self.coms = np.zeros((buffer_size, n_envs, 3), dtype=np.float32)
        self.coms_truth = np.zeros((buffer_size, n_envs, 3), dtype=np.float32)
        self.hidden_states_com = np.zeros(self.hidden_state_shape, dtype=np.float32)
        self.cell_states_com = np.zeros(self.hidden_state_shape, dtype=np.float32)
 
    def reset(self):
        super().reset()
        self.coms = np.zeros((self.buffer_size, self.n_envs, 3), dtype=np.float32)
        self.coms_truth = np.zeros((self.buffer_size, self.n_envs, 3), dtype=np.float32)
        self.hidden_states_com = np.zeros(self.hidden_state_shape, dtype=np.float32)
        self.cell_states_com = np.zeros(self.hidden_state_shape, dtype=np.float32)


    def add(self, obs, actions, rewards, dones, values, log_probs, lstm_states, coms, coms_truth):
        # Add data to the original buffer
        self.coms[self.pos] = coms
        self.coms_truth[self.pos] = coms_truth
        self.hidden_states_com[self.pos] = np.array(lstm_states.com[0].cpu().numpy())
        self.cell_states_com[self.pos] = np.array(lstm_states.com[1].cpu().numpy())
        super().add(obs, actions, rewards, dones, values, log_probs, lstm_states=lstm_states)
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RecurrentRolloutBufferSamples, None, None]:
        ...
        if not self.generator_ready:
            ...
            for tensor in ["hidden_states_pi", "cell_states_pi", "hidden_states_vf", "cell_states_vf", "hidden_states_com", "cell_states_com"]:
    ...
    def _get_samples(
    ...
    ) -> RecurrentRolloutBufferSamples2:
        ...
        lstm_states_com = (
            # (n_envs * n_steps, n_layers, dim) -> (n_layers, n_seq, dim)
            self.hidden_states_com[batch_inds][self.seq_start_indices].swapaxes(0, 1),
            self.cell_states_com[batch_inds][self.seq_start_indices].swapaxes(0, 1),
        )
        ...
        lstm_states_com = (self.to_torch(lstm_states_com[0]).contiguous(), self.to_torch(lstm_states_com[1]).contiguous())

        return RecurrentRolloutBufferSamples2(
            ...
            lstm_states=RNNStatesCOM(lstm_states_pi, lstm_states_vf, lstm_states_com),
            episode_starts=self.pad_and_flatten(self.episode_starts[batch_inds]),
            coms=self.pad_and_flatten(self.coms[batch_inds]),
            coms_truth=self.pad_and_flatten(self.coms_truth[batch_inds]),
...
class RecurrentPPO(OnPolicyAlgorithm):
    ...
    def __init__(
        ...
        n_steps: int = 8, #128, # 1024, # 128,
        batch_size: Optional[int] = 64, #128,
        n_epochs: int = 4, #10,
        gamma: float = 0.99,
    ...
    def _setup_model(self) -> None:
        ...
        #buffer_cls = ExtendedRecurrentRolloutBuffer if isinstance(self.observation_space, spaces.Dict) else RecurrentRolloutBuffer
        buffer_cls =  ExtendedRecurrentRolloutBuffer
        ...
        # hidden and cell states for actor and critic and COM
        self._last_lstm_states = RNNStatesCOM (
            (
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                actions, values, log_probs, com, lstm_states = self.policy.forward(obs_tensor, lstm_states, episode_starts)
                #print(com)
    ...
    def train(self) -> None:
        ...
        entropy_losses = []
        pg_losses, value_losses, com_losses = [], [], []
        clip_fractions = []
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                #COM loss
                pred_com = rollout_data.coms
                true_com = rollout_data.coms_truth
                #true_com_tensor = th.from_numpy(true_com).float().to(pred_com.device)  # Ensure the tensor is on the same device as pred_com
                com_loss = th.mean(((true_com -  pred_com)**2 ))
                com_losses.append(com_loss.item())
                #print(com_loss)

                loss = policy_loss + self.ent_coef * entropy_loss + self.vf_coef * value_loss + self.com_coef * com_loss
